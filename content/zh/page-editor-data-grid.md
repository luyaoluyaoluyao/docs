...
本文档描述Mendix Studio中的数据格。

数据网格显示一个表格式的对象列表。 例如，数据网格可以显示事件的所有程序项目。 使用控制，最终用户可以浏览、搜索和编辑数据网格中的对象。
...
*  **显示分页控制** - 表示是否可以看到通过数据网格分页的按钮。 如果您确定数据网格只包含一个页面则隐藏这些按钮
...
*  **按钮** — — 列出控制栏中的按钮。 在 **事件** 部分中设置为 **单击动作** 按钮将被标记为 *点击动作。* 这意味着，即使按钮被隐藏， 当最终用户点击或双击(取决于 [**事件**](#grid-events) 部分的设置)网格项目， 默认按钮 (动作) 将被触发
...
在 **单击动作** 部分中，您可以选择一个数据网格的特定动作来按钮 (例如) 取消选中所有或导出到 Excel，或标准一个 (例如，执行微流程或保存更改)。 关于点击动作的按钮和标准的更多信息，请参阅 [按钮小部件](page-editor-widgets-buttons) and [小部件中的事件部分](page-editor-widgets-events-section)。
...
* **选择所有** - 允许最终用户在网格中选择所有对象； 只能在您设置 **[选区模式](#selection-mode)** 到 **多选区** 或 **单击多选区** (更多信息) See section [2. 事件](#grid-events))
...
| **选择模式** <a name="selection-mode"></a> | 确定最终用户在数据网格中选择项目的方式。 此属性的可能值如下： <ul><li>**没有选择** - 用户不能选择项目</li><li>**单个选择** - 用户可以一次通过单击选择一个项目</li><li>**总是一个选择** - 用户可以一次点击它选择一个项目。 然而，用户不能取消选择一个项目 (默认第一个项目将被选择)</li><li>**多选** - 用户可以通过点击第一个选项，按住 <kbd>Ctrl</kbd> 键并点击其他项目来选择多个项目。 只需单击项目就可以取消选择所有项目并使单击项目成为选中项目。</li><li>**单击多个选择** - 用户可以单击它们来选择多个项目。 单击一个项目将取消选择所有项目并使单击项目成为选中项目。</li></ul>                                                 |




### 2.7 设计 {#grid-design-section}
