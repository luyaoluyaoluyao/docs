...
Mendix有许多自定义设置，其中大多数都在Mendix参考指南中的 [自定义设置](/refguide7/custom-settings) 中描述。
...
| 名称             | 描述                                                                                                       | 默认值  |
| -------------- | -------------------------------------------------------------------------------------------------------- | ---- |
| `启用 KeepAlive` | 定义为防止会话超时是否在 `会话超时`/2 毫秒，Web 客户端是否发送了存活请求。 浏览器中的每个单击都可以使用 `KeepAlive`。 禁用此属性将导致用户在10分钟闲置后自动注销，即使浏览器仍然开启。 | true |
...
| 名称         | 描述                                                               | 默认值    |
| ---------- | ---------------------------------------------------------------- | ------ |
| `会话超时`     | 定义会话在多长时间后无效(毫秒)。 在这段时间过后，届会将适用于除名。 直到下次集群管理员评估活动会话时，会话才会被摧毁。    | 600000 |
| `集群管理动作间隔` | 用于执行所有集群管理器操作的间隔(毫秒)。 这些动作包括解除对用户的屏蔽和移除无效会话。 如果没有指定，则间隔为 `会期超时`。 | 300000 |
...
由于Mendix 7采用了无国籍的跑道，由于两个原因，内存使用导致问题的可能性已经减少。 第一个原因是能够在横向环境中运行。 多运行时将意味着意外的内存使用量在运行时也会被分割，减少任何空闲用户会话的影响。 但主要(和第二个)原因是，大多数内存使用情况已转移到客户端。 因此而不是内存中的所有实体最终在应用程序节点上， 一大部分最终将在客户端的浏览器中。 这将大大减少应用程序节点可能因增加 `会期超时` 默认值到更高的值而造成的压力。 如果您在 Mendix 6 中设置超时值很高， 您应该考虑使用一个比您其他使用更大的节点尺寸。
...
| 名称                 | 描述                                                                                                                                                       | 默认值 |
| ------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------- | --- |
| `LogMinDuration查询` | 定义数据库查询是否通过 `ConnectionBus_Queries` 日志节点登录，如果它们在这里指定的毫秒数后完成。 默认情况下，只记录关于 SQL 查询的记录。 将 `ConnectionBus_Queries` 日志节点的日志级别设置为 TRACE 以显示更多有关表格或导致此查询的微流程的信息。 |     |

`LogMinDurationQuery` 可能是一个非常有用的工具，用于检测比预期时间长的查询。 对于仅在应用程序中使用的数据增长和由应用程序使用的数据增长后需要更长时间的查询，这特别有用。 因为这可能意味着查询只会在几个月的使用后变得较慢，而且可能还没有在发布前的性能测试中出现。 确定查询速度慢取决于您正在运行的应用程序类型。 但一般来说，任何直接影响到使用应用程序的用户的查询(意味着， 不会是一个后台进程，而会有一个较低的阈值来阻止它比在后台运行的查询更慢。 例如， 需要5秒加载才能选择任何东西的下拉菜单比后台生成的 PDF 需要8秒而不是4秒的差很多，因为需要5秒而不是1秒。
...
* [自定义设置](/refguide7/custom-settings)
