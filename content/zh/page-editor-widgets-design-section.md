...
| Align Self      | 在布局元素中对齐部件。 关于布局元素的更多信息，请参阅 [结构部件](page-editor-widgets-structure) | **自我对齐** 的可能值如下：<ul><li>无 — — 将元素置于前一个元素旁边。</li><li>左对齐元素左边(将元素放置在布局元素左边)</li><li>右对齐元素到右(将元素放置在布局元素右侧)</li></ul>                               |
...
### 2.1 列表视图特定财产 {#list-view-design-properties}
...
### 2.2 集装箱特定属性 {#container-design-properties}
...
### 2.3 按钮特定属性 {#button-design-properties}
...
| 全宽度        | 按钮占用它放置的布局元素的整个宽度。 欲了解更多布局元素的信息，请参阅 [结构部件](page-editor-widgets-structure)。                                                                                                                                                                                                                                                                                         | 启用/禁用                                          |
...
| Align Icon | 将图标移动到文本右侧或顶部。                                                                                                                                                                                                                                                                                                                                                     | **对齐图标** 的可能值如下：<ul><li>无 -- 图标在文本左侧</li><li>右-图标移动到按钮或链接的右边</li><li>顶部 - 图标位于按钮* 样式的按钮文本上方左边的 *link* 样式的左边(关于按钮样式的更多信息) 查看 [按钮小部件](page-editor-widgets-butons))</li></ul>      |

### 2.4 特定文本属性 {#text-design-properties}

**文本** 部件的特定属性在下表中描述：

| 财产   | 描述         | 可能的值                                                                                                                                                              |
| ---- | ---------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 权重   | 更改字体的权重    | **重量** 的可能值如下：<ul><li>无</li><li>亮色的</li><li>普通(如果重量设置为 **无** 为默认值)</li><li>Semibold</li><li>Bold</li></ul>                                                                                                                           |
