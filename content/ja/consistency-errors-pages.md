...
例えば、 *顧客* という名前のページには、マイクロフローを呼び出すボタンがあります (ボタンの [クリックイベント](on-click-event) は *Microflow* に設定されています)。
...
ボタンや画像など、さまざまなウィジェットに [イベント](on-click-event) を指定できます。

最も一般的な整合性エラーは、クリックイベントを構成していないと接続されます。

整合性エラーを修正するには、クリックイベントの構成を完了します (例: for an on click event **Show a page**, select a specific page that should open), or change the on click event.
