...
{{% alert type="info" %}}
<img src="attachments/chinese-translation/china.png" style="display: inline-block; margin: 0" /> 簡体字中国語の翻訳については、 [<unk> <unk> <unk>](https://cdn.mendix.tencent-cloud.com/documentation/refguide8/project-settings.pdf) をクリックしてください。
{{% /alert %}}
...
### ユーザーあたり3.11 複数のセッション{#multiple-sessions}
...
このオプションは2つの異なる値を持つことができます: **ランタイム** と **データベース**。 **データベース** がデフォルト値になります。
...
**Runtime** が選択されている場合、属性と関連付けの一意性は Mendix Runtime で処理され、データベース レベルでは処理されません。
...
[Deprecated] プロジェクトのテーマを設定するための ZIP ファイルの使用は推奨されません。 [UI リソース パッケージ](ui-resources-package) は、テーマを共有するための好ましい方法です。
...
## 7つのその他のタブ {#miscellaneous}

これらの設定は、このプロジェクトの Studio Pro の動作を決定します。 このプロジェクトに取り組んでいるすべての人に設定が適用されます。

### ローカル実行時の7.1 バンドルウィジェット
...
### 7.2 マイクロフロー内の小文字変数名を提案

有効にすると、Studio Pro がマイクロフローで提案する名前は、大文字ではなく小文字で始まります。

### 7.3アクティビティのデフォルトの色

この表では、プロジェクトで使用可能な各マイクロフローアクティビティタイプの既定の色を選択できます。 選択した色は、プロジェクト内のそのタイプのすべてのマイクロフローアクティビティの背景色として使用されます。 マイクロフローエディタで個々のアクティビティのこのデフォルト値を上書きすることができます。 アクティビティタイプのデフォルトの色を変更する場合 プロジェクトには個別の背景色を指定した活動があります これらのアクティビティにも新しい既定の色を適用できるダイアログが表示されます。
