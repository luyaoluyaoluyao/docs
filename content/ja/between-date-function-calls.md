---
title: "日付関数呼び出し間隔"
parent: "表現"
menu_order: 100
tags:
  - "studio pro"
  - "日付関数呼び出しの間に"
  - "表現"
  - "表現"
---

{{% alert type="info" %}}
<img src="attachments/chinese-translation/china.png" style="display: inline-block; margin: 0" /> 簡体字中国語の翻訳については、 [<unk> <unk> <unk>](https://cdn.mendix.tencent-cloud.com/documentation/refguide8/between-date-function-calls.pdf) をクリックしてください。
{{% /alert %}}

## 1つの紹介

日付関数の呼び出しの間で、2 つの日付の違いを計算します。

DateTime 関数の使い方を詳しく調べるには、以下のビデオをご覧ください。

<img
  style="width: 100%; margin: auto; display: block;"
  class="vidyard-player-embed"
  src="https://videoshare.mendix.com/watch/Cjy2qSeKcGhcuKEmkKesp2?.jpg"
  data-uuid="Cjy2qSeKcGhcuKEmkKesp2"
  data-v="4"
  data-type="inline"
 />

## 2 ミリ秒間隔

`ミリ秒間の` 関数は、ミリ秒単位で入力値の差を計算します。

### 2.1 入力パラメータ

入力パラメータは以下の表に記載されています:

| 値      | タイプ   |
| ------ | ----- |
| 最初の日付  | 日付と時刻 |
| 2回目の日付 | 日付と時刻 |

### 2.2 出力

出力は以下の表に記載されています:

| 値                   | タイプ     |
| ------------------- | ------- |
| ミリ秒単位で測定された2つの日付の違い | 小数点以下桁数 |

### 2.3 例

次の入力を使用する場合:

```java
millisecondsBetween(dateTime(2007, 1, 1, 1, 1), dateTime(2007,1,1,1,1,3))
```

出力は:

```java
2000
```

## 3 秒の間

`秒間` 関数は、入力値の差を秒単位で計算します。

### 3.1 入力パラメータ

入力パラメータは以下の表に記載されています:

| 値      | タイプ   |
| ------ | ----- |
| 最初の日付  | 日付と時刻 |
| 2回目の日付 | 日付と時刻 |

### 3.2 出力

出力は以下の表に記載されています:

| 値                     | タイプ     |
| --------------------- | ------- |
| 2つの日付の違いは、秒単位で測定されます。 | 小数点以下桁数 |

### 3.3 例

次の入力を使用する場合:

```java
secondsBetween(dateTime(2007, 1, 1, 1, 1), dateTime(2007,1,1,1,2,3))
```

出力は:

```java
62
```

## 4 分間

`minutes Between` 関数は、入力値の差を分単位で計算します。

### 4.1 入力パラメータ

入力パラメータは以下の表に記載されています:

| 値      | タイプ   |
| ------ | ----- |
| 最初の日付  | 日付と時刻 |
| 2回目の日付 | 日付と時刻 |

### 4.2 出力

出力は以下の表に記載されています:

| 値                    | タイプ     |
| -------------------- | ------- |
| 2つの日付の違いは分単位で測定されます。 | 小数点以下桁数 |

### 4.3 例

次の入力を使用する場合:

```java
minutes Between(dateTime(2007, 1, 1, 1, 2, 1), dateTime(2007,1,1,1,1,1))
```

出力は:

```java
1
```

## 5時間の間

`hoursBetween` 関数は、時間単位の入力値の差を計算します。

### 5.1 入力パラメータ

入力パラメータは以下の表に記載されています:

| 値      | タイプ   |
| ------ | ----- |
| 最初の日付  | 日付と時刻 |
| 2回目の日付 | 日付と時刻 |

### 5.2 出力

出力は以下の表に記載されています:

| 値                   | タイプ     |
| ------------------- | ------- |
| 時間単位で測定された2つの日付の違い。 | 小数点以下桁数 |

### 5.3 例

次の入力を使用する場合:

```java
hoursBetween(dateTime(2007, 1, 1, 3, 311), dateTime(2007,1,1,1,1,1))
```

出力は:

```java
2.5
```

## 6日前

`daysBetween` 関数は、日数の入力値の差を計算します。

### 6.1 入力パラメータ

入力パラメータは以下の表に記載されています:

| 値      | タイプ   |
| ------ | ----- |
| 最初の日付  | 日付と時刻 |
| 2回目の日付 | 日付と時刻 |

### 6.2 出力

出力は以下の表に記載されています:

| 値                                                                                                 | タイプ        |
| ------------------------------------------------------------------------------------------------- | ---------- |
| 2つの日付の違いは、日数で測定された正の小数として。 時間は考慮されるので、 `date-x 0:00` と `date-x 06:00` を比較すると、 `0.25000000` となります。 | 正の小数点以下の桁数 |

### 6.3 例

次の入力を使用する場合:

```java
daysBetween(dateTime(2007, 2, 13, 1, 1, 1), dateTime(2007,1,1,1,11))
```

出力は:

```java
43
```

## 7 週間の間

`週間` 関数は、数週間の入力値の差を計算します。

### 7.1 入力パラメータ

入力パラメータは以下の表に記載されています:

| 値      | タイプ   |
| ------ | ----- |
| 最初の日付  | 日付と時刻 |
| 2回目の日付 | 日付と時刻 |

### 7.2 出力

出力は以下の表に記載されています:

| 値                  | タイプ     |
| ------------------ | ------- |
| 数週間で測定された二つの日付の違い。 | 小数点以下桁数 |

### 7.3 例

次の入力を使用する場合:

```java
weeksBetween(dateTime(2007, 1, 9, 1, 1, 1), dateTime(2007,1,1,1,1,1))
```

出力結果は週に7日で8日で分割されます。

```java
1.1428571428571428
```

## 8 カレンダーの間で

{{% alert type="warning" %}}この式は Mendix バージョン 8.15 以降でのみ使用できます。{{% /alert %}}

`calendar MonthsBetween` 関数は月単位の入力値の差を計算します。 時間は無視されます。 日付は、エンドユーザーのローカルタイムゾーンにあると想定されます。

### 8.1 入力パラメータ

入力パラメータは以下の表に記載されています:

| 値      | タイプ   |
| ------ | ----- |
| 最初の日付  | 日付と時刻 |
| 2回目の日付 | 日付と時刻 |

### 8.2 出力

出力は以下の表に記載されています:

| 値                     | タイプ |
| --------------------- | --- |
| 2つの日付の違いは、数ヶ月で測定されます。 | 整数  |

### 8.3 例

次の入力を使用する場合:

```java
calendarMonthsBetween(dateTime(2007年、1、10、1、1)、dateTime(2007年、8、1、1、1、1))
```

出力は、2つの日付の間の月数になります:

```java
7
```

## 9 カレンダー年の間

{{% alert type="warning" %}}この式は Mendix バージョン 8.15 以降でのみ使用できます。{{% /alert %}}

`calendar Years` 関数の間では、入力値の年数の差を計算します。 時間は無視されます。 日付は、エンドユーザーのローカルタイムゾーンにあると想定されます。

### 9.1 入力パラメータ

入力パラメータは以下の表に記載されています:

| 値      | タイプ   |
| ------ | ----- |
| 最初の日付  | 日付と時刻 |
| 2回目の日付 | 日付と時刻 |

### 9.2 出力

出力は以下の表に記載されています:

| 値                   | タイプ |
| ------------------- | --- |
| 二つの日付の違いは年間で測定されます。 | 整数  |

### 9.3 例

次の入力を使用する場合:

```java
calendarYearsBetween(dateTime(2007年1, 1, 10, 1), dateTime(2017年1, 1, 1, 1, 1))
```

出力は、2つの日付の間の年数になります:

```java
10
```
